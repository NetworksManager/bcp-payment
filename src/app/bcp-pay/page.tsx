...
export default function BcpPaymentPage() {
  ...
  return (
    <div className="max-w-md mx-auto p-8 bg-white min-h-screen">
      ...
      {publicKey && !success && (
        <button onClick={handlePayment} disabled={loading} className="w-full bg-orange-600 text-white py-4 rounded-xl font-bold text-lg disabled:bg-orange-300">
      ...
      {success && (
        <div className="bg-green-100 border border-green-500 rounded-2xl p-8 text-center">
          ...
          {nftAddress ? (
            <div className="mb-4">
              <p className="text-green-700 font-medium mb-3">Your generative NFT ticket has been minted!</p>
              <a href={`https://www.tensor.trade/item/${nftAddress}`} target="_blank" className="inline-block bg-black text-white px-6 py-3 rounded-xl font-semibold mb-2">
                View on Tensor →
              </a>
              <br />
          ...
          <button onClick={() => window.location.href = "https://bitcoinpalooza.nyc"} className="mt-2 bg-green-700 text-white px-8 py-3 rounded-xl">
